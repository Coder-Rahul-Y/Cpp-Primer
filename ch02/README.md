...
C++ guarantees `short` and `int` is **at least** 16 bits, `long` **at least** 32 bits, `long long` **at least** 64 bits of space in memory.

The `signed` can represent positive numbers, negative numbers and zero, while `unsigned` can only represent positive numbers and zero. Also, the largest positive number that can be stored in `unsigned` is twice that which can be stored in `signed` type.
